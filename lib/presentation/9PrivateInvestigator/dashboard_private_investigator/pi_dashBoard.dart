...
import 'package:virtual_experts/model_final/pi_models/all_private_investigator_data_model.dart';
...
class piDashboardScreen extends StatefulWidget {
  const piDashboardScreen({super.key});

  @override
  State<piDashboardScreen> createState() => _piDashboardScreenState();
}

class _piDashboardScreenState extends State<piDashboardScreen> {
  ...
  _getData() async {
    String endpoint = "http://${ApiServices.ipAddress}/all_private_investigator_data";
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        data.addAll(
            result.map((e) => AllPrivateInvestigatorData.fromJson(e)).toList().reversed);
