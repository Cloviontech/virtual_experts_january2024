...
class SmCompleteProfileScreen extends StatefulWidget {
  const SmCompleteProfileScreen({super.key});

  @override
  State<SmCompleteProfileScreen> createState() => _SmCompleteProfileScreenState();
}

class _SmCompleteProfileScreenState extends State<SmCompleteProfileScreen> {
...
Future<void> profileUpload(BottomNavigationSalesScreen bottomNavigationSalesScreen)async {
  // var uri = "http://${ApiServices.ipAddress}/ad_pro_upload_account/$userId";
  var uri = "http://${ApiServices.ipAddress}/sm_upload_account/$userId";
  print(userId);
  ...
  if(response.statusCode == 200){
    print("sm data add successfully : ${response.statusCode}");
    Navigator.push(context, MaterialPageRoute(builder: (context)=>bottomNavigationSalesScreen));
    // push;
  }
}
                  ...
                  child: TextButton(
                      onPressed: (){
                        if((_formKey.currentState?.validate()??false) && degreeCertificate != null && experienceCertificate !=null && gstCertificate !=null && panCard !=null){
                          ...
                          profileUpload(BottomNavigationSalesScreen());
                        }
                        else {
                          setState(() {
