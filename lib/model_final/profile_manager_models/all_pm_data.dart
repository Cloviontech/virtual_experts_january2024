...
//
//     final allPmData = allPmDataFromJson(jsonString);

import 'dart:convert';

List<AllPmData> allPmDataFromJson(String str) => List<AllPmData>.from(json.decode(str).map((x) => AllPmData.fromJson(x)));

String allPmDataToJson(List<AllPmData> data) => json.encode(List<dynamic>.from(data.map((x) => x.toJson())));
...
class AllPmData {
    ...
    factory AllPmData.fromJson(Map<String, dynamic> json) => AllPmData(
        uid: json["uid"],
        email: json["email"],
        mobile: json["mobile"],
